...
export function AdvancedReports(){
  ...
  const visible=useMemo(()=>{const now=new Date(),today=now.toISOString().slice(0,10),startOfWeek=new Date(now);startOfWeek.setDate(now.getDate()-((now.getDay()+6)%7));const weekStart=startOfWeek.toISOString().slice(0,10),month=today.slice(0,7),year=today.slice(0,4);return mail.filter(m=>period==='all'||(period==='today'&&m.date===today)||(period==='week'&&m.date>=weekStart&&m.date<=today)||(period==='month'&&m.date.startsWith(month))||(period==='year'&&m.date.startsWith(year))||(period==='custom'&&(!fromDate||m.date>=fromDate)&&(!toDate||m.date<=toDate))).filter(m=>!filterType||m.type===filterType).filter(m=>!filterDepartment||m.department===filterDepartment).filter(m=>!filterStatus||normalizeMailStatus(m.status)===filterStatus).filter(m=>!filterPriority||m.priority===filterPriority)},[mail,period,fromDate,toDate,filterType,filterDepartment,filterStatus,filterPriority]);
  const departmentsFilter=[...new Set(mail.map(m=>m.department).filter(Boolean))],statusesFilter=[...new Set(mail.map(m=>normalizeMailStatus(m.status)))];
  const total=visible.length||1;
  const completed=visible.filter(m=>isFinalMailStatus(m.status)).length;
  ...
  const statuses=[...new Set(visible.map(m=>normalizeMailStatus(m.status)))].map(status=>({status,count:visible.filter(m=>normalizeMailStatus(m.status)===status).length}));
  ...
  const reportMail=reportType==='followup'?visible.filter(m=>!isFinalMailStatus(m.status)&&(m.workflow.length||[MAIL_STATUSES.IN_PROGRESS,MAIL_STATUSES.WAITING_REPLY,MAIL_STATUSES.REFERRED].includes(normalizeMailStatus(m.status) as never))):reportType==='deadlines'?visible.filter(m=>deadlineState(m.dueDate,m.status)==='overdue'):reportType==='closed'?visible.filter(m=>isFinalMailStatus(m.status)):visible;
  ...
  const missingOwner=visible.filter(m=>!m.employee||!m.department),missingDue=visible.filter(m=>!m.dueDate&&!isFinalMailStatus(m.status)&&normalizeMailStatus(m.status)!==MAIL_STATUSES.SENT),withoutFiles=visible.filter(m=>!m.attachments.length),exceptions=visible.filter(m=>deadlineState(m.dueDate,m.status)==='overdue');
  ...
  return <>
    ...
    <div className="report-grid">
      <section className="panel"><div className="panel-head"><div><h2>توزيع الحالات</h2><p>عدد المراسلات حسب المرحلة الحالية</p></div></div>{statuses.map(x=><div className="bar" key={x.status}><div><Status>{x.status}</Status><b>{x.count}</b></div><i><em style={{width:`${x.count/total*100}%`}}/></i></div>)}</section>
    ...
    <div className="report-grid">
      <section className="panel"><h2>قياس الالتزام</h2><div className="kpi-list"><div><span>مراسلات منجزة</span><b>{completed}</b></div><div><span>متأخرة</span><b>{visible.filter(m=>normalizeMailStatus(m.status)===MAIL_STATUSES.OVERDUE).length}</b></div><div><span>بانتظار الرد</span><b>{visible.filter(m=>normalizeMailStatus(m.status)===MAIL_STATUSES.WAITING_REPLY).length}</b></div></div></section>
